...
from dict_hub import get_rel_dict
from util_lar import get_before_get_lar
...
def get_rel_rel(rel: str) -> str:
    prefix = rel.split(' ')[0]
    if prefix == 'inverse':
        rel = ' '.join(rel.split(' ')[1:])
#         rel_desc = rel_dict.get_dict_by_rel(rel).inver_rel_desc
        rel_desc = rel_dict.get_dict_by_rel(rel).rel_desc
        real_rel = rel_dict.get_dict_by_rel(rel).ori_relation
        # rel = rel_dict.get_dict_by_rel(rel).rel_inverse
    else:
        rel_desc = rel_dict.get_dict_by_rel(rel).inver_rel_desc
#         rel_desc = rel_dict.get_dict_by_rel(rel).rel_desc
        real_rel = rel_dict.get_dict_by_rel(rel).rel_inverse
    return real_rel, rel_desc
...
#         return entity_dict.get_entity_by_id(self.head_id).entity_desc if (entity_dict.get_entity_by_id(self.head_id).if_new == 0) else entity_dict.get_entity_by_id(self.head_id).llm_desc

    @property
    def tail_desc(self):
        return entity_dict.get_entity_by_id(self.tail_id).entity_desc
#         return entity_dict.get_entity_by_id(self.tail_id).entity_desc if (entity_dict.get_entity_by_id(self.tail_id).if_new == 0) else entity_dict.get_entity_by_id(self.tail_id).llm_desc
    ...
    def vectorize(self) -> dict:
        ...
        if self.relation == '':
            ...
            rel, rel_desc = get_rel_rel(self.relation)
            rel_text = _concat_name_desc(rel, rel_desc)
            # rel_text = rel

        hr_encoded_inputs = _custom_tokenize(text=head_text,
                                             text_pair=rel_text) # self.relation, rel_text
