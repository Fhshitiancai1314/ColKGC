...
def compute_metrics(hr_tensor: torch.tensor,
    ...
    for start in tqdm.tqdm(range(0, total, batch_size)):
        ...
        sorted_score_list.append(batch_sorted_score)
        sorted_indices_lsit.append(batch_sorted_indices)
    ...
    sorted_indices_lsit = torch.cat(sorted_indices_lsit, dim=0)
    sorted_score_list = torch.cat(sorted_score_list, dim=0)
    ...
    return topk_scores, topk_indices, metrics, ranks, sorted_score_list, sorted_indices_lsit
...
def eval_single_direction(predictor: BertPredictor,
    ...
    hr_tensor, _, ex_list = predictor.predict_by_examples(examples)
    ...
    topk_scores, topk_indices, metrics, ranks, sorted_score_list, sorted_indices_lsit = compute_metrics(hr_tensor=hr_tensor, entities_tensor=entity_tensor,
                                                                target=target, examples=examples,
                                                                batch_size=batch_size)
    if eval_forward:
        torch.save(sorted_score_list, '/E22301291/SimKGCv2/llm_rerank/forward/score_list.pt')
        torch.save(sorted_indices_lsit, '/E22301291/SimKGCv2/llm_rerank/forward/index_list.pt')
        with open('/E22301291/SimKGCv2/llm_rerank/forward/ex_list.json', 'w') as f:
            json.dump(ex_list, f)
        f.close()
        with open('/E22301291/SimKGCv2/llm_rerank/forward/target.json', 'w') as f:
            json.dump(target, f)
        f.close()
    else:
        torch.save(sorted_score_list, '/E22301291/SimKGCv2/llm_rerank/back/score_list.pt')
        torch.save(sorted_indices_lsit, '/E22301291/SimKGCv2/llm_rerank/back/index_list.pt')
        with open('/E22301291/SimKGCv2/llm_rerank/back/ex_list.json', 'w') as f:
            json.dump(ex_list, f)
        f.close()
        with open('/E22301291/SimKGCv2/llm_rerank/back/target.json', 'w') as f:
            json.dump(target, f)
        f.close()
