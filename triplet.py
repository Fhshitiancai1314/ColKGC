...
class EntityDict:
    ...
    def __init__(self, entity_dict_dir: str, inductive_test_path: str = None):
        path = os.path.join(entity_dict_dir, 'entsv4.json')  # entities.json，ents.json
...
class relDict:
    def __init__(self, entity_dict_dir: str, inductive_test_path: str = None):
        path = os.path.join(entity_dict_dir, 'relv5.json')  # entities.json
        assert os.path.exists(path)
        self.entity_exs = [relExample(**obj) for obj in json.load(open(path, 'r', encoding='utf-8'))]
        ...
        self.rel2entity = {ex.relation: ex for ex in self.entity_exs}
        self.rel2idx = {ex.relation: i for i, ex in enumerate(self.entity_exs)}
        logger.info('Load {} entities from {}'.format(len(self.rel2entity), path))

    def get_dict_by_rel(self, rel: str) -> EntityExample:
        return self.rel2entity[rel]
